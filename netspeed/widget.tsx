...
function tone(mbps: number) {
  if (mbps >= 50) return { icon: 'bolt.fill', color: '#34C759' }
  if (mbps >= 10) return { icon: 'hare.fill', color: '#007AFF' }
  return { icon: 'tortoise.fill', color: '#FF9500' }
}
  ...
  return (
    ...
    >
      <VStack alignment="center" spacing={small ? 6 : 8} padding={small ? 12 : 16} frame={{ maxWidth: 'infinity', maxHeight: 'infinity' }}>
        ...
        <HStack spacing={0} frame={{ width, height: 4 }} background={{ light: '#E8E8ED', dark: '#3A3A3C' }} clipShape={{ type: 'rect', cornerRadius: 2 }}>
